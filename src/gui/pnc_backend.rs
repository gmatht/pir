...
pub fn run_pancurses(app: &mut super::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Store cursor cell raw value at the cursor's position for formula bar lookup
    {
        ...
        // Show the cursor cell's raw value in the formula bar (normal mode),
        // matching ratatui's formula bar which displays the cell value without
        // entering edit mode until the user explicitly types.
        // No set_editing call here — the widget remains in normal mode and
        // reads the cell value from the raw_cells store for the formula bar.
    }
