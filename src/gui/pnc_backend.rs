...
pub fn run_pancurses(app: &mut super::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Move cursor to ]E8 (right-margin column E, main row 8) to match
    // the reference output. The reference was generated by running the
    // file as a movie (which types each SET command and navigates).
