...
// ---------- TabView (pancurses: minimal; renderer already supports tab_titles) ----------
use std::rc::Rc;
use std::cell::{RefCell, Cell};

pub struct TabView {
    pub(crate) content: BoxWidget,
    pub(crate) titles: Rc<RefCell<Vec<String>>>,
...
impl AsRef<*mut std::ffi::c_void> for TabView {
    fn as_ref(&self) -> &*mut std::ffi::c_void { self.content.as_ref() }
}

impl TabView {
    pub fn new(_parent: *mut std::ffi::c_void) -> Result<Self, crate::core::Error> {
        let content = create_box(Orientation::Vertical, 0)?;
        Ok(TabView {
            content,
    ...
    pub fn add_tab(&self, title: &str) -> Result<usize, crate::core::Error> {
        let idx = self.titles.borrow().len();
        self.titles.borrow_mut().push(title.to_string());
        Ok(idx)
    }
    pub fn tab_box(&self, _idx: usize) -> Result<BoxWidget, crate::core::Error> {
    ...
    pub fn set_active(&self, idx: usize) {
        let idx = idx.min(self.titles.borrow().len().saturating_sub(1));
        self.active.set(idx);
        if let Some(cb) = self.tab_changed.borrow_mut().as_mut() { cb(idx); }
    }
    pub fn current_tab(&self) -> usize { self.active.get() }
    pub fn tab_count(&self) -> usize { self.titles.borrow().len() }
    pub fn tab_title(&self, idx: usize) -> Option<String> {
        self.titles.borrow().get(idx).cloned()
    }
    pub fn set_tab_title(&self, idx: usize, title: &str) {
        if let Some(t) = self.titles.borrow_mut().get_mut(idx) { *t = title.to_string(); }
    }
    pub fn close_tab(&self, idx: usize) {
        let count = self.titles.borrow().len();
        if idx >= count { return; }
        self.titles.borrow_mut().remove(idx);
        // Keep `active` pointing at a valid tab after the shift.
        let new_active = if self.titles.borrow().is_empty() {
            0
        } else if idx >= count - 1 {
            count - 2
        } else {
    ...
    pub fn tab_titles(&self) -> Vec<String> { self.titles.borrow().clone() }
}
