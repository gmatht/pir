...
//
// `GetWindowSubclass` is NOT exported by comctl32.dll (absent on Windows 11
// 24H2), so importing it makes any MSVC binary fail to load with
// STATUS_ENTRYPOINT_NOT_FOUND. Reimplemented with a private map of the
// subclass reference data set via `SetWindowSubclass` instead.
//
